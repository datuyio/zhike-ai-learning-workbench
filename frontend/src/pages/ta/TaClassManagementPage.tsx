...
type ClassForm = { name: string; description: string; max_students: string; course_id: string };
...
export function TaClassManagementPage(): JSX.Element {
  ...
  const [form, setForm] = useState<ClassForm>({ name: '', description: '', max_students: '', course_id: '' });
  ...
  const filteredClasses = searchQuery.trim()
    ? allClasses.filter((c) => c.name.toLowerCase().includes(searchQuery.trim().toLowerCase()))
    : allClasses;
  const totalStudents = allClasses.reduce((sum, c) => sum + c.student_count, 0);
  const courseName = (courseId: string | null): string => {
    if (!courseId) return '通用';
    return coursesQuery.data?.find((c) => c.id === courseId)?.title ?? '通用';
  };
  ...
  const saveMutation = useMutation({
    mutationFn: async (): Promise<{ id: string; invite_code?: string }> => {
      ...
      if (editing) {
        return taUpdateClass(editing.id, payload);
      }
      return taCreateClass(payload);
    },
    onSuccess: (result) => {
  ...
  function openCreate(): void {
    setEditing(null);
    setForm({ name: '', description: '', max_students: '', course_id: '' });
    setFormError(null);
    setFormOpen(true);
  }
  ...
  async function exportGrades(item: TaClass): Promise<void> {
    try {
      ...
      setActionError(`导出「${item.name}」成绩失败：${(error as Error).message}`);
    }
  }
    ...
    <div className="mx-auto w-full max-w-6xl">
      ...
      <PageHeaderToolbar>
        ...
        <span className="text-xs text-zinc-500">
          共 {allClasses.length} 个班级 · {totalStudents} 名学生
        </span>
      </PageHeaderToolbar>
      ...
      {actionError ? (
        <div className="mb-4 flex items-center justify-between gap-3 rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
          ...
          <button type="button" className="rounded-md px-2 py-1.5 text-xs text-red-700 hover:bg-red-100" onClick={() => setActionError(null)}>✕</button>
        </div>
      ) : null}
          ...
          <table className="w-full text-left text-sm">
            <thead className="border-b border-zinc-200 bg-zinc-50 text-xs text-zinc-500">
              <tr>
                ...
                <th className="px-4 py-3 font-medium">关联课程</th>
            ...
            <tbody className="divide-y divide-zinc-100">
              {filteredClasses.map((item) => (
                <tr key={item.id} className="transition-colors hover:bg-zinc-50">
                  ...
                  <td className="px-4 py-3 text-zinc-600">{courseName(item.course_id)}</td>
                  <td className="px-4 py-3">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4">
          <div className="w-full max-w-md rounded-lg border border-zinc-200 bg-white p-5 shadow-xl">
            ...
            <div className="mt-4 space-y-3">
              ...
              <label className="block text-xs text-zinc-500">
                关联课程
                <select className={`${inputClass} mt-1`} value={form.course_id} onChange={(e) => setForm({ ...form, course_id: e.target.value })}>
                  <option value="">不关联（通用班级）</option>
                ...
                <li key={student.id} className="flex items-center justify-between py-2.5">
                  <div>
                    ...
                    <div className="text-xs text-zinc-400">
                      ...
                      {student.joined_at ? ` · 加入于 ${formatBeijingDateTimeCompact(student.joined_at, '—')}` : ''}
                    </div>
                  </div>
                  <button
                    type="button"
